...
if (isset($_POST['shopping-cart'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        echo "<script>alert('Produk sudah ada di keranjang!'); window.location.href = 'printa3+.php';</script>";
    } else {
        ...
        if (mysqli_stmt_affected_rows($insert_cart) > 0) {
            echo "<script>alert('Produk ditambahkan ke keranjang!'); window.location.href = 'printa3+.php';</script>";
        } else {
            echo "<script>alert('Gagal menambahkan produk ke keranjang!'); window.location.href = 'printa3+.php';</script>";
        }
    }
...
<body>
    ...
    <section class="produk">
        ...
        <div class="produk-item">
            <?php
            include("../php/config.php");
            $result = mysqli_query($con, "SELECT * FROM produk WHERE kategori = 'printa3'");
